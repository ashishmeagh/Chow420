...
class DisputeController extends Controller
{
                               ...
                               )
    {
        ...
        $this->module_view_folder = "buyer/dispute";
    ...
    public function index($order_id="")
    {
     ...
     //  $order_details   = $this->OrderModel->with('seller_details')->where('id',$order_id)->first()->toArray(); 
        $order_details   = $this->OrderModel->with('seller_details.seller_detail','buyer_details')->where('id',$order_id)->first(); 
        ...
        if($user_details->inRole('buyer'))
        {
           //get buyer details
            ...
            if($buyer_obj)
            {
                $buyer_details = $buyer_obj->toArray(); 


                $sender_id     = $buyer_details['user_id'];
        ...
        $this->arr_view_data['buyer_details']        = $buyer_details;
    ...
    //function for buer dispute raise
        ...
        { 
          ...
          // $url     = url('/').'/'.config('app.project.admin_panel_slug').'/dispute/'.base64_encode($order_details['id']);
           ...
           $url     = url('/').'/'.config('app.project.admin_panel_slug').'/order';
            ...
            // return response()->json(['status' => 'success', 'description' => $user_details]);
                $admin_role = Sentinel::findRoleBySlug('admin');        
                $admin_obj = \DB::table('role_users')->where('role_id',$admin_role->id)->first();
                if($admin_obj)
                {
                ...
                $arr_event['description']  = html_entity_decode('Buyer <b>'.$user_name.'</b> has raised the dispute for order with Order No.: <a target="_blank" href="'.$url.'"><b>'.$order_details['order_no'].'</b></a>.');
                $arr_event['type']         = '';
                $arr_event['title']        = 'New Dispute Raised';
         ...
         /*******************Notification END For Admin   ***************************/
           ...
           /* $msg     = html_entity_decode('Buyer '.$user_name.' has raised the dispute for order with Order No.: '.$order_details['order_no'].'.');*/
            ...
            /************************send notification and email to seller**************************/
               // $sellerurl     = url('/').'/seller/dispute/'.base64_encode($order_details['id']);

                $sellerurl     = url('/').'/seller/order/completed';

                $arr_event                 = [];
                $arr_event['from_user_id'] = $from_user_id;
                $arr_event['to_user_id']   = $receiver_id;
                $arr_event['description']  = html_entity_decode('Buyer <b>'.$user_name.'</b> has raised the dispute for order with Order No.: <a target="_blank" href="'.$sellerurl.'"><b>'.$order_details['order_no'].'</b></a>.');
                $arr_event['type']         = '';
                $arr_event['title']        = 'New Dispute Raised';
                ...
                //email to seller
                $to_user = Sentinel::findById($receiver_id);

                $f_name  = isset($to_user->first_name) ?$to_user->first_name : '';
                $l_name  = isset($to_user->last_name) ?$to_user->last_name : '';
                $user_email  = isset($to_user->email) ?$to_user->email : '';
             ...
             if(!empty($user_details))
             {
               ...
               if($role!="" && $role=='admin')
               {
                  /*$chat_arr    = $this->DisputeChatModel->with(['sender_details','receiver_details'])
                    ...
                    $chat_arr    = $this->DisputeChatModel->with(['sender_details.seller_detail','receiver_details'])
                                   //  ->where('order_id',$orderid)
                                    // ->where('order_no',$orderno)
         ...
         /*********************Send Email Notification START***********************/
            ...
            $chat_msg = '';
             ...
             if(isset($file_name) && $file_name != Null){
                ...
                $chat_msg = html_entity_decode('Buyer '.$user_name.' has sent message on dispute chat for Order No <b>'.$order_no.'</b><br> Message: '.$message);
            }
            ...
            $arr_event['to_user_id']   = $receiver_id;
            $arr_event['description']  = html_entity_decode('Buyer '.$user_name.' has sent message on dispute chat for order no: <a target="_blank" href="'.$dispute_url.'">'. $obj_order->order_no.'</a>.');
            $arr_event['type']         = '';
