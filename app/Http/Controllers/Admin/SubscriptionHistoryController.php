...
class SubscriptionHistoryController extends Controller
{
               ...
               )
    {
        ...
        $this->module_view_folder = "admin.subscription-history";
        $this->module_url_path    = $this->admin_url_path."/seller_membership_history";  
    ...
    public function index($enc_id = NULL)
    {
        ...
        $this->arr_view_data['module_title']        = 'Membership History';

        $this->arr_view_data['module_url_path']   = $this->module_url_path;
        $this->arr_view_data['seller_id']         = isset($seller_id)?$seller_id:'';

        return view($this->module_view_folder.'.index',$this->arr_view_data);
    }

    public function get_payment_history(Request $request,$sellerid)
        {  
            ...
            $obj_qutoes = DB::table($usersubscripiton_table)
                                    ...
                                    ->where($usersubscripiton_table.'.user_id',$sellerid)
            ...
            $json_result  = $json_result->editColumn('created_at',function($data) use ($current_context)
                             ...
                             ->editColumn('cancel_reason',function($data) use ($current_context)
                            {
                                if(isset($data->cancel_reason) && strlen($data->cancel_reason)>20){
                                   return '<p class="prod-desc">'.str_limit($data->cancel_reason,30).'..<a class="readmorecancelbtn" cancel_reason="'.$data->cancel_reason.'" style="cursor:pointer"><b>Read more</b></a></p>';
                                }
                                else{
