...
class ShopByEffectController extends Controller
{       
    use MultiActionTrait;

    public function __construct(ShopByEffectModel $ShopByEffectModel,
                                EventModel $EventModel,
                                UserService $UserService

                               ) 
    {
    	$this->BaseModel          = $ShopByEffectModel;
        ...
        $this->shop_by_effect_base_img_path   = base_path().config('app.project.img_path.shop_by_effect');
        $this->shop_by_effect_public_img_path = url('/').config('app.project.img_path.shop_by_effect');

        $this->module_title       = "Shop By Reviews ";
        $this->module_view_folder = "admin.shop_by_effect";
        $this->module_url_path    = $this->admin_url_path."/shop_by_reviews";
    } 
 

    public function index()
    {
        $shop_by_effect_table          = $this->BaseModel->getTable();

        $shop_by_effect_content_count = DB::table($shop_by_effect_table)
                                     ->count();
                                 // dd($obj_shop_by_effect_table);
        $this->arr_view_data['page_title']        = "Manage ".$this->module_title;
        $this->arr_view_data['module_title']      = str_singular($this->module_title);
        $this->arr_view_data['module_url_path']   = $this->module_url_path;
        $this->arr_view_data['admin_panel_slug']  = config('app.project.admin_panel_slug'); 
        $this->arr_view_data['shop_by_effect_content_count']  = $shop_by_effect_content_count;
    ...
    {
    	$shop_by_effect_table          = $this->BaseModel->getTable();
        $prefixed_shop_by_effect_table = DB::getTablePrefix().$this->BaseModel->getTable();

        $obj_shop_by_effect_table = DB::table($shop_by_effect_table)                           
        							 ->orderBy($shop_by_effect_table.'.created_at','DESC');

       
        $current_context = $this;

        $json_result = Datatables::of($obj_shop_by_effect_table);  
        ...
        $json_result =   $json_result->editColumn('enc_id',function($data) use ($current_context)
                        ...
                        ->editColumn('image',function($data) use ($current_context)
                        {
                            if($data->image != "")
                            {
                               return '<img src="'.$this->shop_by_effect_public_img_path.'/'.$data->image.'" alt="image" class="img-responsive" width="50%" height="50%"/> ';
                            }
                            else
                            {
                                return "N/A";
    ...
    public function store(Request $request)
    {
        ...
        /* Main Model Entry */
        $shop_by_effect_instance = $this->BaseModel->firstOrNew(['id' => $id]);

        $shop_by_effect_instance->link_url  = isset($form_data['link_url'])?$form_data['link_url']:'';
        $shop_by_effect_instance->title      =  isset($form_data['title'])?$form_data['title']:'';
        $shop_by_effect_instance->subtitle   =  isset($form_data['subtitle'])?$form_data['subtitle']:'';
        ...
        if($request->is_active!='' && !empty($request->is_active)){
         ...
         $shop_by_effect_instance->is_active = $is_active;
        ...
        {	
            ...
            $image1->save($this->shop_by_effect_base_img_path.'/'.$file_name);

            $unlink_old_img_path    = $this->shop_by_effect_base_img_path.'/'.$request->input('old_img');
        ...
        $shop_by_effect_instance->image  = $file_name;   


      
        $shop_by_effect_instance->save();

        if($shop_by_effect_instance)
        {
    ...
    {       
        ...
        if($obj_data)
        {
      	...
      	$this->arr_view_data['shop_by_effect_public_img_path']     = $this->shop_by_effect_public_img_path;
        ...
        if($multi_action=="delete")
        {
            /*-------------------------------------------------------
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has performed multiple delete operation on shop by effect.';
        ...
        if($multi_action=="activate")
        {
             /*-------------------------------------------------------
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has performed multiple activate operation on shop by effect.';
        ...
        if($multi_action=="deactivate")
        {
              /*-------------------------------------------------------
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has performed multiple deactivate operation on shop by effect.';
    ...
    public function perform_delete($id,$flag=false)
    {
        ...
        if($entity)
        {
            ...
            $unlink_old_img_path    = $this->shop_by_effect_base_img_path.'/'.$entity->image;
            ...
            if($flag==false)
            {
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has deleted the shop by effect '.$entity_arr['title'].'.';
        ...
        if($entity)
        {
            ...
            if($flag == false)
            {
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has activated the shop by effect: '.$entity_arr['title'].'.';
        ...
        if($entity)
        {
            ...
            if($flag==false)
            {
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has deactivated the shop by effect:  '.$entity_arr['title'].'.';
