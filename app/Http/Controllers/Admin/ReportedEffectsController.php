...
class ReportedEffectsController extends Controller
{       
    use MultiActionTrait;

    public function __construct(ReportedEffectsModel $ReportedEffectsModel,
                                EventModel $EventModel,
                                UserService $UserService

                               ) 
    {
    	$this->BaseModel          = $ReportedEffectsModel;
        ...
        $this->reported_effects_base_img_path   = base_path().config('app.project.img_path.reported_effects');
        $this->reported_effects_public_img_path = url('/').config('app.project.img_path.reported_effects');

        $this->module_title       = "Reported Effects";
        $this->module_view_folder = "admin.reported_effects";
        $this->module_url_path    = $this->admin_url_path."/reportedeffect";
    } 
    ...
    {
    	$reported_news_table          = $this->BaseModel->getTable();
        $prefixed_productnews_table = DB::getTablePrefix().$this->BaseModel->getTable();

        $obj_productnews_table = DB::table($reported_news_table)                           
        							 ->orderBy($reported_news_table.'.created_at','DESC');
        ...
        $json_result =   $json_result->editColumn('enc_id',function($data) use ($current_context)
                        ...
                        ->editColumn('image',function($data) use ($current_context)
                        {
                            if($data->image != "")
                            {
                               return '<img src="'.$this->reported_effects_public_img_path.'/'.$data->image.'" alt="image" class="img-responsive" width="10%" height="10%"/> ';
                            }
                            else
                            {
                                return "N/A";
    ...
    public function store(Request $request)
    {
        ...
        $is_duplicate = $this->BaseModel->where('title',$form_data['title']);
        ...
        if($alredy_exist > 0)
        {
            ...
            $response['description']='Reported effect is already exists';
        ...
        if($request->hasFile('image'))
        {
           ...
           if(!in_array($file_extension,['svg']))
           {
                $response['status'] ='error';
                $response['description']='Please select valid image only .svg images allowed';
                return response()->json($response);
           }
        }       
        ...
        /* Main Model Entry */
        $reported_effects = $this->BaseModel->firstOrNew(['id' => $id]);

        $reported_effects->title      =  isset($form_data['title'])?$form_data['title']:'';
             ...
             if($extension != 'svg'){
                   /* $imageThmb = Image::make($uploadPath.'/'.$imageName);
                    $imageThmb->fit(300,300,function($constraint){$constraint->upsize();})->save($uploadPath.'/thumbs/thm_'.$imageName,80);*/
                        ...
                        $image1->save($this->reported_effects_base_img_path.'/'.$file_name);
                }else{
                    //dd(public_path());
                    $file_name   = uniqid(rand(11111,99999)).'.'.$extension;
                    $uploadPath = 'uploads/up/';
                    $file->move($uploadPath,$file_name);
                    File::copy($uploadPath.'/'.$file_name,$this->reported_effects_base_img_path.'/'.$file_name);
                    File::delete('uploads/up/'.$file_name);

                }           
           

            $unlink_old_img_path    = $this->reported_effects_base_img_path.'/'.$request->input('old_img');
        ...
        $reported_effects->image  = $file_name;   
        ...
        if($request->is_active!='' && !empty($request->is_active)){
         ...
         $reported_effects->is_active = $is_active;
      
        $reported_effects->save();
        ...
        {            
            ...
            if($is_update_process == false)
            {
                 ...
                 /***********Add Event data*for reportedeffects****/
                    ...
                    // $arr_eventdata['message']  = 'Checkout the <b>'.$form_data['title'].'</b> on reported effects .<div class="clearfix"></div><a href="'.url('/').'/reportedeffects" target="_blank" class="viewcls">View</a>';

                     $arr_eventdata['message']  = '<div class="discription-marq">
                      <div class="mainmarqees-image">
                         <img src="'.url('/').'/assets/front/images/chow.png" alt="">
                       </div>Checkout the <b>'.$form_data['title'].'</b> on reported effects .<div class="clearfix"></div></div><a href="'.url('/').'/reportedeffects" target="_blank" class="viewcls">View</a>';


                    $arr_eventdata['title']    = 'Reported Effects';                        
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has added reported effects '.$reported_news->title.'.';
            ...
            {     
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has updated reported effects '.$reported_news->title.'.';
    ...
    {       
        ...
        if($obj_data)
        {
           $arr_reported_news = $obj_data->toArray(); 
        }
    
      	$this->arr_view_data['reported_effects_public_img_path']     = $this->reported_effects_public_img_path;
      	$this->arr_view_data['edit_mode']                = TRUE;
     	$this->arr_view_data['enc_id']                   = $enc_id;
      	$this->arr_view_data['module_url_path']          = $this->module_url_path;
      	$this->arr_view_data['arr_reported_news']   		 = isset($arr_reported_news) ? $arr_reported_news : [];  
        ...
        if($multi_action=="delete")
        {
            /*-------------------------------------------------------
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has performed multiple delete operation on reported effects.';
        ...
        if($multi_action=="activate")
        {
             /*-------------------------------------------------------
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has performed multiple activate operation on reported effects.';
        ...
        if($multi_action=="deactivate")
        {
              /*-------------------------------------------------------
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has performed multiple deactivate operation on reported effects.';
    ...
    public function perform_delete($id,$flag=false)
    {
        ...
        if($entity)
        {
            ...
            $unlink_old_img_path    = $this->reported_effects_base_img_path.'/'.$entity->image;
            ...
            if($flag==false)
            {
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has deleted reported effects '.$entity_arr['title'].'.';
        ...
        if($entity)
        {
            ...
            if($flag == false)
            {
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has activated reported effects '.$entity_arr['title'].'.';
        ...
        if($entity)
        {
            ...
            if($flag == false)
            {
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has featured reported effects '.$entity_arr['title'].'.';
        ...
        if($entity)
        {
            ...
            if($flag==false)
            {
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has deactivated reported effects '.$entity_arr['title'].'.';
        ...
        if($entity)
        {
            ...
            if($flag==false)
            {
                ...
                if(isset($user) && $user->inRole('sub_admin'))
                {
                    ...
                    $arr_event['title']        = $this->module_title;
                    $arr_event['user_id']      = isset($user->id)?$user->id:'';
                    $arr_event['message']      = $user->first_name.' '.$user->last_name.' has unfeatured reported effects '.$entity_arr['title'].'.';
