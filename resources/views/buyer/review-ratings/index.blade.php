          ...
          @php
           ...
           <div class="col-xs-6 col-sm-6 col-md-4 col-lg-3 rating-cal">
             ...
             <div class="top-rated-brands-list favoiritelist my-review-ratings money-back-link-main-bx">
                ...
                {{-- @if(isset($product['age_restriction']) && $product['age_restriction']!="" && isset($product['age_restriction_detail']['age']) && $product['age_restriction_detail']['age']!="")
                  <div class="label-list">{{$product['age_restriction_detail']['age']}}</div>
                @endif --}}

                @if(isset($product['first_level_category_details']['age_restriction']) && $product['first_level_category_details']['age_restriction']!="" && isset($product['first_level_category_details']['age_restriction_detail']['age']) && $product['first_level_category_details']['age_restriction_detail']['age']!="")
                  <div class="label-list">
                   {{$product['first_level_category_details']['age_restriction_detail']['age']}}
                  </div>
                @endif

                {{-- <a title="{{isset($product['product_name'])?$product['product_name']:''}}" href="{{url('/')}}/search/product_detail/{{ isset($product['id'])?base64_encode($product['id']):''}}/{{ $product_slug }}/{{ $review_brandname }}/{{ $review_sellername }}" > --}}
                <a title="{{isset($product['product_name'])?$product['product_name']:''}}" href="{{url('/')}}/search/product_detail/{{ isset($product['id'])?base64_encode($product['id']):''}}/{{ $product_slug }}" >



                <div class="img-rated-brands">
                    <div class="thumbnailss">
                        <img src="{{$product_img}}" class="portrait" alt="{{isset($product['product_name'])?$product['product_name']:''}}"  onclick="return productclick({{ isset($pageclick)?json_encode($pageclick):''}})"/>
                    </div>
                    <div class="content-brands">
                          <div class="title-sub-list">{{isset($product['first_level_category_details']['product_type'])?$product['first_level_category_details']['product_type']:''}}
                          </div>

                        {{--  <div class="price-listing"><span>${{isset($product['unit_price'])?number_format($product['unit_price'],2):''}}</span></div> --}}
                         ...
                         <!---------------show price here------------------>
                           ...
                           <!-------------end of brand name----------------------->
                           {{-- <a title="{{isset($product['product_name'])?$product['product_name']:''}}" href="{{url('/')}}/search/product_detail/{{ isset($product['id'])?base64_encode($product['id']):''}}/{{ $product_slug }}/{{ $review_brandname }}/{{ $review_sellername }}"  onclick="return productclick({{ isset($pageclick)?json_encode($pageclick):''}})"> --}}
                           <a title="{{isset($product['product_name'])?$product['product_name']:''}}" href="{{url('/')}}/search/product_detail/{{ isset($product['id'])?base64_encode($product['id']):''}}/{{ $product_slug }}"  onclick="return productclick({{ isset($pageclick)?json_encode($pageclick):''}})">


                        <div class="titlebrds">{{isset($product['product_name'])?$product['product_name']:''}}</div>
                    ...
                    @if($review_str=='Add Review') 
                      ...
                      <a product_id="{{isset($product['id'])?$product['id']:''}}"  onclick="LoadAddReviewModal($(this));" class="view-review-links">{{$add_review_str or ''}}</a>

                    @else
                      <a product_id="{{isset($product['id'])?$product['id']:''}}"  onclick="LoadViewReviewModal($(this));"  class="view-review-links">{{$review_str}}</a> 


                      <a product_id="{{isset($product['id'])?$product['id']:''}}"  onclick="LoadEditReviewModal($(this));"  class="view-review-links edit_review pull-left"> Edit Review</a>
                    ...
                    @if(isset($request_arr['status']) && $request_arr['status'] == 0) 
                      
                      <div class="money-back-pendding">Reported</div> 
                    
                    @elseif(isset($request_arr['status']) && $request_arr['status'] == 2)
                      
                      <a href="javascript:void(0);" product_id="{{isset($product['id'])?$product['id']:''}}" id="btn_money_back_request" class="view-review-links money-back-link" onclick="showModel($(this));">Report Issue</a>

                      <div class="clearfix"></div>

                      <div class="money-back-requested" title="{{$request_arr['note'] or ''}}">Corrected</div>

                    @elseif(isset($request_arr['status']) && $request_arr['status'] == 1)  
     ...
     @else
       <div class="empty-product-main">
           ...
           <div class="buyers-btn-mn">
                    <a href="{{url('/')}}/search" class="butn-def">Continue Shopping</a>
                </div>
       </div>
     @endif
    ...
    <div class="modal-dialog ordercancellationmodal">
        ...
        <div class="modal-content">
            ...
            <!-- <div class="ratings-frms">
              ...
              @php /* @endphp <div class="checkbox-dropdown" >
                Helped with
                ...
                <ul class="checkbox-dropdown-list" id="modalchkboxes">
                  ...
                  <li>
                     <div class="checkbox clearfix">
                        <div class="form-check checkbox-theme">
                            <input class="form-check-input" type="checkbox" value="{{ $v['id'] }}" id="rememberMe{{ $v['id'] }}"  name="emoji[]">
                            <label class="form-check-label" for="rememberMe{{ $v['id'] }}">
                              {{ $v['title'] or '' }}
    ...
    <div class="modal-dialog ordercancellationmodal">
        ...
        <div class="modal-content">
           ...
           <div class="title-buyers-review-views-rw" id="title"></div>
            ...
            <div id="topreportedlabelid" style="display: none"> This helped with my:  {{-- <span class="colorbk" data-toggle="tooltip" data-html="true" title="Disclaimer: Effects and flavors are reported by users on our site. This is for informational purposes only and not intended as medical advice. Please consult with your physician before changing medical treatments.  These statements have not been evaluated by the Food and Drug Administration. These products are not intended to diagnose, treat, cure or prevent any disease."><i class="fa fa-info-circle" aria-hidden="true"></i></span> --}}
            ...
            success:function(response)
            {
              ...
              { 
                if(response.emoji)
                {
                   ...
                   //       emojiimg += '<li><img src={{ url('/') }}/assets/images/'+emoji_arr[i]+'  width="32px"/> <label>'+capitalizeFirstLetter(em[0].replace('_',' '))+'</label> </li>' ;
                   //    } 
                   //    emojiimg +='</ul>';
                   // }
                   ...
                   if(emoji_arr)
                   {
                     ...
                     var emojiimg = '<ul class="list-inline">';
                     for (i = 0; i < emoji_arr.length; i++) {
                         emojiimg += '<li><img src={{ url('/') }}/uploads/reported_effects/'+emoji_arr[i]['image']+'  width="32px"/> <label>'+emoji_arr[i]['title']+'</label> </li>' ;
                      } 
                      emojiimg +='</ul>';
                   }
