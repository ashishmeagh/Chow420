...
<table width="100%" bgcolor="#fff" border="0" cellspacing="0" cellpadding="0" style="border:1px solid #ddd;font-family:Arial, Helvetica, sans-serif;">
                             ...
                             <tr>
                               ...
                               <tr>
                                     {{--  <td width="50%" style="font-size:12px;">  --}}
                                      <td width="50%"> 
                                        {{-- <h3 style="margin-bottom: 5px;">Customer Shipping Address</h3> --}}
  ...
  <tr>
    ...
    @endphp
     ...
     @php // for delivery option data

      $seller_delivery_amt_total = 0; $deliverday='';
     if(isset($order_deliveryoptiondata) && !empty($order_deliveryoptiondata)){
        
         foreach($order_deliveryoptiondata as $kk11=>$vv11)
         {
           if(isset($vv11['sellername']) && $vv11['sellername']!='' && isset($vv11['delivery_title']) && $vv11['delivery_title']!='' && isset($vv11['delivery_cost']) && $vv11['delivery_cost']!='' &&  isset($vv11['delivery_day']) && $vv11['delivery_day']!='')
          {
 ...
 @endphp  
    ...
    <tr>
        ...
        <td width="20%" style="text-align: center; font-size:13px; font-weight: bold; border-bottom:1px solid #e5e9f1; padding:12px; text-transform: uppercase; color: #000;">
           ...
           @php 
              $fulltotal =0;
              if(isset($sum) && isset($shipping_charges_sum) && isset($seller_totalall_amt))
              {
                $fulltotal =  $sum + $shipping_charges_sum - $seller_totalall_amt;
              }
              else
              { 
              ...
              {
                $fulltotal =  $fulltotal + $seller_delivery_amt_total;
              }
              else
              { 
                $fulltotal =  $fulltotal;
