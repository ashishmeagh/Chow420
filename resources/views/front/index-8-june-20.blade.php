...
{{-- 
<div class="marquee">
   <div class="toprtd non-mgts viewall-btns-idx toprated-view">Real Time
     <div class="clearfix"></div>
   </div>
    <ul class="marquee-content">
           ...
           @php
            ...
            { 
               if(isset($events['msg'])){
               echo "<li>". $events['msg'].' <div class="time-ago-class"> '.\Carbon\Carbon::parse($events['created_at'])->diffForHumans() ." </div> </li>";
...
{{-- <div class="promotions-updatepg featuredviewallbutton">
  <div class="toprtd viewall-btns-idx viewalls smallfonts featuredcategories-space">
           <span>Featured Categories</span> 
           <a href="#" class="butn-def" title="View all">View All</a>
</div>
...
<!-----------------featured sellers------------------------------->
      ...
      <div class="border-home-brfands"></div>
        ...
        <div class="toprtd viewall-btns-idx">Featured Dispensaries
          <a href="{{ url('/') }}/shopdispensory" class="{{ $class }}" title="View all">View All</a>
          <div class="clearfix"></div>
        </div>
        ...
        >          
          ...
          <div class="images-vertcls">
                  ...
                  @endif
                    ...
                    <div class="content-brands">                       
                        <div class="titlebrds">{{ isset( $featured_seller['seller_detail']['business_name'])?ucwords($featured_seller['seller_detail']['business_name']):''}}</div>                           
                    </div>
                </div>
              </a>
            </li>            
...
<!-------------start of todays deals products----------------------------------->

 @if(isset($arr_eloq_todaysdeals_data) && count($arr_eloq_todaysdeals_data)>0)
 <hr>
        <div class="toprtd non-mgts viewall-btns-idx toprated-view">Today's Deals 

          @php
          if(isset($arr_eloq_todaysdeals_data) && count($arr_eloq_todaysdeals_data)<=$device_count)
          $class = 'butn-def viewall-product';
          else
          $class = 'butn-def';  
          @endphp
        ...
        <div class="featuredbrands-flex trendingproducts-section tprtd-procudt">         
          <ul @if(isset($arr_eloq_todaysdeals_data) && count($arr_eloq_todaysdeals_data)<=$device_count)    
             class="f-cat-below7"      
           {{--  @elseif(isset($arr_product) && $isMobileDevice==1) --}}
          ...
          {{--  @elseif(isset($arr_product) && count($arr_product)>$device_count && $isMobileDevice==0)
              id="flexiselDemo1"
           @endif --}}
            @elseif(isset($arr_eloq_todaysdeals_data) && count($arr_eloq_todaysdeals_data)>$device_count)
              id="flexiselDemo121"
           @endif
         >
            
            @foreach($arr_eloq_todaysdeals_data as $product)
              @php 
                      /* $avg_rating = $product['avg_rating'];
                       $avg_rating  = round_rating_in_half($avg_rating);
                       if($avg_rating==1){$rating = 'one';}
                       ...
                       if($avg_rating==0.5){$rating = 'zeropointfive';}else if($avg_rating==1.5){$rating = 'onepointfive';}else if($avg_rating==2.5){$rating = 'twopointfive';}else if($avg_rating==3.5){$rating = 'threepointfive';}else if($avg_rating==4.5){$rating = 'fourpointfive';}
                        ...
                        $total_reviews = get_total_reviews($product['id']);  
                        if($total_reviews>0)
                          $total_reviews = $total_reviews;
                        else
                           $total_reviews = '';   
             ...
             <li>
                <div class="top-rated-brands-list">
                   @php
                      ...
                      $product_title_slug = str_slug($product_title);
                    @endphp
                    <a href="{{url('/')}}/search/product_detail/{{isset($product['id'])?base64_encode($product['id']):'0'}}/{{ $product_title_slug }}"  title="{{ ucfirst($product['product_name']) }}">
                      <div class="img-rated-brands">
                      ...
                      <!------------start---age restriction div------------>
                        ...
                        @if(isset($product['is_age_limit']) && $product['is_age_limit'] == 1 && isset($product['age_restriction']))
                            <div class="label-list">
                              @php
                                if(isset($product['age_restriction_detail']['age']) && $product['age_restriction_detail']['age']!='')
                               {
                              @endphp
                                 {{ $product['age_restriction_detail']['age']}}
                              @php 
                               }
                              @endphp
                             </div>
                      ...
                      <div class="thumbnailss">
                            ...
                            @if(!empty($product['product_images_details'][0]['image']) && file_exists(base_path().'/uploads/product_images/'.$product['product_images_details'][0]['image']))
                               <img src="{{url('/')}}/uploads/product_images/{{ $product['product_images_details'][0]['image'] }}" class="portrait" alt="{{ ucfirst($product['product_name']) }}" />
                            @else
                               <img src="{{url('/assets/images/default-product-image.png')}}" class="portrait" alt="{{ ucfirst($product['product_name']) }}" />
                            @endif
                        </div>
                        <div class="content-brands">                          
                              ...
                              <div class="titlebrds by-ttlbrn"><span>{{ isset($product['get_brand_detail']['name'])? ucwords($product['get_brand_detail']['name']):''  }}</span></div>  

                            <div class="titlebrds spacetoptitlebrds">{{ ucwords($product['product_name'])  }}</div> 
                            <div class="starthomlist"  @if($avg_rating>0) title="{{$avg_rating}} Avg. rating"
                             @endif>
                                @if($avg_rating>0 && isset($rating))
                                <img src="{{url('/')}}/assets/front/images/star-rate-{{$rating}}.png" alt="star-rate-{{$rating}}.png" title="{{$avg_rating}} Avg. rating"/>
                              
                                <!---------show review----commented temporary----->
      ...
      /*code to stop video after modal closed*/
       ...
       // var screenWidth = document.documentElement.clientWidth;
        ...
        if(parseInt(screenWidth) < parseInt(768)){
          $("#flexiselDemo1, #flexiselDemo2, #flexiselDemo5, #flexiselDemo3,#flexiselDemo125, #flexiselDemo86,#flexiselDemo121").removeAttr('id');
